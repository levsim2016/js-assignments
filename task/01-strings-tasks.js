...
function extractNameFromTemplate(value) {
    var regExpression = /Hello,\s*(\w+)\s*(\w+)!/;
    var matchedStrings = regExpression.exec(value);
...
function getRectangleString(width, height) {
    ...
    for(var y = 0; y < height; y++){
        switch(y){
            case 0:
                fillSymbol = '─';
                result += `┌${ fillSymbol.repeat(innerWidth) }┐\n`;
                break;
...
function encodeToRot13(str) {
    // var encodedCharCodes = Array
    //     .from(str)
    //     .map(character => {
    //         var firstLetterCharCode = 'a'.charCodeAt(0);
    //         var charCode = character.charCodeAt(0) + 13;
            
    //         if(charCode > firstLetterCharCode + 26){
    //             return charCode - 26;
    //         }
    //         return charCode;
    //     });
    
    // return String.fromCharCode(...encodedCharCodes);
